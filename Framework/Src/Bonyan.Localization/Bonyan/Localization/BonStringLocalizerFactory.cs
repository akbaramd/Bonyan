...
public class BonStringLocalizerFactory : IStringLocalizerFactory, IBonStringLocalizerFactory
{
    protected internal BonLocalizationOptions BonLocalizationOptions { get; }
    ...
    public BonStringLocalizerFactory(
        ResourceManagerStringLocalizerFactory innerFactory,
        IOptions<BonLocalizationOptions> abpLocalizationOptions,
        ...
        IExternalLocalizationStore externalLocalizationStore)
    {
        ...
        BonLocalizationOptions = abpLocalizationOptions.Value;
    ...
    {
        var resource = BonLocalizationOptions.Resources.GetOrNull(resourceType);
        if (resource == null)
        {
    ...
    private IStringLocalizer? CreateByResourceNameOrNullInternal(
        ...
        bool lockCache)
    {
        var resource = BonLocalizationOptions.Resources.GetOrDefault(resourceName);
        if (resource == null)
        {
    ...
    {
        foreach (var globalContributorType in BonLocalizationOptions.GlobalContributors)
        {
        ...
        return new StringLocalizerCacheItem(
            new BonDictionaryBasedStringLocalizer(
                resource,
                resource
                    .BaseResourceNames
    ...
    {
        services.Replace(ServiceDescriptor.Singleton<IStringLocalizerFactory, BonStringLocalizerFactory>());
        services.AddSingleton<ResourceManagerStringLocalizerFactory>();
    }

    protected class StringLocalizerCacheItem
    {
        public BonDictionaryBasedStringLocalizer Localizer { get; }

        public StringLocalizerCacheItem(BonDictionaryBasedStringLocalizer localizer)
        {
            Localizer = localizer;
        }
    }
    ...
    {
        if (BonLocalizationOptions.DefaultResourceType == null)
        {
            return null;
        }

        return Create(BonLocalizationOptions.DefaultResourceType);
    }
}