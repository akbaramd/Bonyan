    ...
    public class AccountController : Controller
    {
        ...
        private async Task<List<Claim>> CreateUserClaimsAsync(Domain.Entities.User user)
        {
            var claims = new List<Claim>
            {
                ...
                new Claim(BonClaimTypes.Name, user.Profile.FirstName.ToString()),
                new Claim(BonClaimTypes.SurName, user.Profile.LastName.ToString()),
                new Claim(BonClaimTypes.UserId, user.Id.ToString()),
                new Claim(BonClaimTypes.UserName, user.UserName),
                new Claim(BonClaimTypes.SessionId, Guid.NewGuid().ToString())
            };
            ...
            if (rolesResult.IsSuccess)
            {
                ...
                {
                    claims.Add(new Claim(ClaimTypes.Role, role.Id.Value));
                    claims.Add(new Claim("RoleTitle", role. Title));
                }
            }
            ...
            try
            {
                ...
                {
                    claims.Add(new Claim(BonPermissionClaimTypes.Permission, permission));
                }
            }
            catch (Exception ex)
            {
